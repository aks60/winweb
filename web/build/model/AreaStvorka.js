...
export class AreaStvorka extends AreaSimple {
    ...
    init_constructiv() {
        ...
        if (this.sysfurnRec == undefined)
            this.sysfurnRec = findef(dbset.sysfurnList.find(rec => rec.list[SYSFURN.systree_id] == this.winc.nuni), dbset.sysfurnList); //ищем первую в системе
        ...
        if (this.wson.param != undefined && this.wson.param.artiklHandl != undefined)
            ...
            this.handleRec = findef(dbset.artiklList.find(rec =>
                this.sysfurnRec[SYSFURN.artikl_id1] == rec.list[ARTIKL.id]), dbset.artiklList); //ручка по умолчанию см. систему
        ...
        if (this.wson.param != undefined && this.wson.param.colorHandl != undefined)
            ...
            this.handleColor = findef(dbset.artdetList.find(rec =>
                this.handleRec[ARTIKL.id] == rec.list[ARTDET.artikl_id]), dbset.artdetList)[ARTDET.color_fk]; //первая запись
            if (this.handleColor < 0)
                this.handleColor = findef(dbset.colorList.find(rec =>
                    (-1 * this.handleColor) == rec.list[COLOR.id]), dbset.colorList)[COLOR.id]; //первый цвет в группе
        }

        //Подвес (петли)
        if (this.wson.param != undefined && this.wson.param.artiklLoop != undefined)
            this.loopRec = findef(dbset.artiklList.find(rec => this.wson.param.artiklLoop == rec.list[ARTIKL.id]), dbset.artiklList);
        ...
        if (this.wson.param != undefined && this.wson.param.artiklLock != undefined)
            this.lockRec = findef(dbset.artiklList.find(rec => this.wson.param.artiklLock == rec.list[ARTIKL.id]), dbset.artiklList);
    ...
    paint() {
        ...
        if (this.winc.root.typeForm() == "DOOR") {
            ...
            let handlRGB = findef(dbset.colorList.find(rec => this.handleColor == rec.list[COLOR.id]), dbset.colorList);
