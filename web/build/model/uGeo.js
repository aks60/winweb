...
UGeo.splitPolygon = (geom, segm) => {
    try {
        ...
        //let segmImp = UGeo.normalizeSegm(jsts.geom.LineSegment.new(
                //[segm.p0.x, segm.p0.y, segm.p0.z],
                //[segm.p1.x, segm.p1.y, segm.p1.z]));
        let segmImp = jsts.geom.LineString.new([[segm.p0.x, segm.p0.y, segm.p0.z], [segm.p1.x, segm.p1.y, segm.p1.z]]);
        
        //Вставим точки пересечения в список коорд.
        const pointloc = new jsts.algorithm.PointLocator();
        for (let i = 1; i < coo.length; i++) {
            let lineSid = jsts.geom.LineString.new([[coo[i - 1].x, coo[i - 1].y], [coo[i].x, coo[i].y]]);//точка пересечения двкх линии 
            let crosP = segmImp.intersection(lineSid);
            
            hsCheck.add(coo[i]);
            if (crosP !== null) {
                let line = jsts.geom.LineString.new([coo[i - 1], coo[i]]);
        ...
        for (let i = 1; i < listExt.length; ++i) {
            ...
            if (Number.isNaN(crd.z)) {
                ...
                let cL = jsts.geom.Coordinate.new(crd.x, crd.y, segmImp.p0.z);
                let cR = jsts.geom.Coordinate.new(crd.x, crd.y);
                if (crosTwo[0] === crd) {
                    cL.z = segmImp.p0.z;
                    cR.z = listExt[i - 1].z;
                } else {
        ...
        return [jsts.geom.LineString.new(crosTwo), jsts.geom.Polygon.new(cooL), jsts.geom.Polygon.new(cooR)];
    } catch (e) {
...
UGeo.geo2Cross = (poly, line) => {
    try {
        ...
        for (let i = 1; i < coo.length; i++) {
            ...
            const segm1 = coo[i - 1];
            const segm2 = coo[i];
            const cros = jsts.algorithm.Intersection.intersection(line.p0, line.p1, segm1, segm2);
            if (cros !== null) {
                out.push(cros);
            }
        }
...
UGeo.getSegment = (poly, index) => {
    ...
    return new jsts.geom.LineSegment(coo[j], coo[k]);
};
