...
export class UColor {
    ...
    static findFromArtOrSeri(spcAdd) {  //см. http://help.profsegment.ru/?id=1107 
        ...
        let typesUS = spcClon.detailRec.getInt(thid.COLOR_US);
    ...
    static colorFromSetting(spcAdd, side, seri) {  //см. http://help.profsegment.ru/?id=1107        
        ...
        try {
            ...
            let elemArtID = spcAdd.artiklRec.getInt(eArtikl.id);

            //Цвет элемента по которому подбираю из варианта подбора
            let originColorID = getID_colorUS(spcAdd, srcColorUS);
            ...
            if (srcColorFk > 0 && srcColorFk !== 100000) {
                ...
                } else if ([UseColor.PROF.id, UseColor.GLAS.id, UseColor.COL1.id, UseColor.COL2.id,
                    UseColor.COL3.id, UseColor.C1SER.id, UseColor.C2SER.id, UseColor.C3SER.id].includes(srcColorUS)) {
                ...
                if ([UseColor.PROF.id, UseColor.GLAS.id].includes(srcColorUS)) {
                ...
                } else if ([UseColor.COL1.id, UseColor.COL2.id, UseColor.COL3.id,
                    UseColor.C1SER.id, UseColor.C2SER.id, UseColor.C3SER.id].includes(srcColorUS)) {
                ...
                } else if ([UseColor.PARAM.id, UseColor.PARSER.id].includes(srcColorUS)) {
                    let parmapRec = eParmap.find3(syspar1Rec.getStr(eSyspar1.text), syspar1Rec.getInt(eSyspar1.groups_id));
    ...
    static  scanFromColorFirst(spc) {
        ...
        if (artdetRec.getInt(1) !== -1) {
            let colorFK2 = artdetRec.getInt(eArtdet.color_fk);
        ...
        alert("Для артикула  " + spc.artikl + " не определена цена.");
        ...
        try {
            switch (srcColorUS) {
                ...
                case 11: //По текстуре профиля
                    let firstElem = spcAdd.elem5e.root.frames.get(0);
    ...
    static scanFromProfSide(detailArtiklID, originColorID, side) {
        try {
            let artdetList = eArtdet.filter(detailArtiklID);
            //Цикл по ARTDET определённого артикула
            for (let artdetRec of artdetList) {
