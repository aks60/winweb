...
kits.load_table = function (table) {
    ...
    if (order.wincalcMap.size != 0) {
        ...
        for (let i = 0; i < kits.prjkitList.length; i++) {
            ...
            let artiklRec = findef(tr[PRJKIT.artikl_id], ARTIKL.id, dbset.artiklList);
            table.jqGrid('addRowData', i + 1, {
                id: tr[KITS.id],
                code: artiklRec[ARTIKL.code],
                name: artiklRec[ARTIKL.name],
                color1: findef(tr[PRJKIT.color1_id], COLOR.id, dbset.colorList)[COLOR.name],
                color2: findef(tr[PRJKIT.color2_id], COLOR.id, dbset.colorList)[COLOR.name],
                color3: findef(tr[PRJKIT.color3_id], COLOR.id, dbset.colorList)[COLOR.name],
