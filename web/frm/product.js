...
product.load_table = function (table1) {
    ...
    let winc = order.wincalcMap.get(order.prprodRec[PROPROD.id]);
...
product.server_to_fields = function () {
    try {
        if (order.prprodRec != undefined) {
            let proprodID = order.prprodRec[PROPROD.id];
            if (proprodID != undefined) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        let id = order.prprodRec[PROPROD.id];
